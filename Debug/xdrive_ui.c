...
//控件(目录)
REINui_Widget_Typedef			ui_main_catalog;		//主目录//_ 过度
...
void REINui_Directory_Append_Widget(REINui_Widget_Typedef* _host, REINui_Widget_Typedef* _slave)
{
	...
	if(_host->feature_pack == NULL)
		return;
	//_非空
	REINui_Directory_Typedef	*directory = (REINui_Directory_Typedef*)_host->feature_pack;//_高级功能包存放的是目录包，主控件的目录包提取出来进行子控件添加。
	//!注意这里的void类型的指针类型明确
	//向主机添加目标控件
	if(directory->target_widget != NULL)//_目标控件指针非空，指向列表的第一个单元
	{
		//寻找主机的下级链接中(同级链接为NULL的单元)//_seek_pointer为目录遍历的指针
		REINui_Widget_Typedef *seek_pointer = (REINui_Widget_Typedef*)directory->target_widget;
		for(;;)
		{
			if(seek_pointer->linked_widget == NULL)//_目录控件的下一个控件不存在
			{
				seek_pointer->linked_widget = (void*)_slave;//_将子控件链接到最后一个控件之后
				break;//_则结束遍历
			}
	...
	//向从机添加源控件
	_slave->source_widget = (void*)_host;//_主控件的地址给 从控件的源地址指向
}
...
void REINui_Switches_drawing_function(void* _widget)
{
	...
	REINui_Switches_Typedef		*switches = (REINui_Switches_Typedef*)widget->feature_pack;//_过渡动画=传入控件的高级功能包
	//先不做动画,直接跳转到目标界面
	ui_activated = switches->end_widget;	//_激活的控件指针=过度动画 的 结束控件
}
...
void REINui_Directory_drawing_function(void* _widget)
{
	...
	//响应刷新指令
	if(widget->update_drawing)//_如果主控件的绘制刷新位有效进行刷新；
	{
		...
		for(uint32_t i=0; i<directory->list_size; i++)
		{
			...
			if(i == ((directory->pitch_number + directory->list_size - 1) % directory->list_size))
				directory->up_widget = seek_pointer;//_ -1 0 1 2 每次只更改三个选项中的一个
			//寻找中项目
			if(i == (directory->pitch_number))
				directory->middle_widget = seek_pointer;//_0 1 2 3
			//寻找下项目
			if(i == ((directory->pitch_number + 1) % directory->list_size))
				directory->down_widget = seek_pointer;//_1 2 3 4

			//读取链表下一个控件
			if(seek_pointer->linked_widget == NULL)
				break;
			seek_pointer = (REINui_Widget_Typedef*)(seek_pointer->linked_widget);//_
		}
		...
		if(directory->list_size >= 2){
			...
			else							slid_length = 30 / (directory->list_size);//_滑块的长度与目录数量的大小成反比
		...
		if(directory->middle_widget->icon_3232 != NULL)
			GRAM_ShowPicture(ssd1306_gram, 96, 0, 32, 32, directory->middle_widget->icon_3232, true);//_图标存放在控件结构体之中
		else
	...
	//用户输入响应
	//_向上移动——通过改变pitch_number的值实现。
	if(Button_Inquice_State(Button_UP) == Button_Bit_Rise){//_根据案件的状态来对主控件的 选中项目编号进行操作
		widget->update_drawing = true;
		if(directory->pitch_number)		directory->pitch_number --; //_如果选中的项目编号不为零  选中的项目编号-1. 
		else							directory->pitch_number = directory->list_size - 1;//_如果选中的项目编号为0  项目编号为最后一个项目的编号
	}
	//_向下移动
	if(Button_Inquice_State(Button_DOWN) == Button_Bit_Rise){//_
		widget->update_drawing = true;
		if((directory->pitch_number + 1) < directory->list_size)	directory->pitch_number ++;
		else														directory->pitch_number = 0;
	}
	//_控件进入退出——通过改变选中项目指针来实现。
	if(Button_Inquice_State(Button_UP) == Button_Bit_LongDrop){
		if(directory->middle_widget != NULL)
		{
			ui_pitch = directory->middle_widget;	//进入选中项目//_ui_pitch 为选中的项目指针
...
void REINui_Desktop_drawing_function(void* _widget)
{
	...
	//_match 在这里会被赋值true或者标志位	
	if(ui_motor_mode == motor_control.mode_order){//_与电机模式相互匹配
		match = true;
		GRAM_ShowPicture(ssd1306_gram, 120, 0, 8, 8, (uint8_t*)Yes_88, !match); //_输出yes false
	}
	else{//_与电机模式不匹配
		match = ui_data.flicker_flag_500ms;	//闪烁   //_match=data.flicker_flag_500ms 的闪烁标志
	...
	else{//_这里可以看出使能的话会闪烁可以推理出ui_data.flicker_flag_500ms是false
...
void XDrive_REINui_Init(void)
{
	...
	//(默认选择"桌面")
	ui_pitch = &ui_desktop;//_选中控件的指针初始化为桌面
...
void XDrive_REINui_Callback_ms(uint32_t _time)
{
	...
	ui_data.flicker_timer_100ms += _time;//_闪烁的时间超过100ms
	if(ui_data.flicker_timer_100ms > 100){
		ui_data.flicker_timer_100ms = 0;//_将闪烁时间更新为0
		ui_data.flicker_flag_100ms = !ui_data.flicker_flag_100ms;//_每隔100msflag取反实现闪烁
	}
