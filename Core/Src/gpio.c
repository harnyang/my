...
#ifdef STALL_SIGNAL_GPIO_CLK_ENABLE
	STALL_SIGNAL_GPIO_CLK_ENABLE();             //启用堵转信号端口时钟
	GPIO_InitStruct.Pin = STALL_SIGNAL_Pin;
	GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;		//推挽输出
    GPIO_InitStruct.Pull = GPIO_NOPULL;				//禁用拉电阻
	GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;	//低速
	HAL_GPIO_Init(STALL_SIGNAL_GPIO_Port, &GPIO_InitStruct);
#endif
}
...
void REIN_GPIO_SIGNAL_COUNT_Init(void)
{
	...
	GPIO_InitStruct.Mode = GPIO_MODE_IT_RISING_FALLING;	//双边沿中断//_这里的双边沿中断决定了后面高低电平都进行检测
	GPIO_InitStruct.Pull = GPIO_NOPULL;									//禁用上下拉
